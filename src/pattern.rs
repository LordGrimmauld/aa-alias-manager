...
impl Pattern {
    ...
    pub(crate) fn find_matches(&self, store_entry: &PathBuf, mut consumer: impl FnMut(&str)) {
        if !store_entry.is_dir() {
            return;
        } // Nothing worth aliasing except directory contents
        ...
        for store_suffix in &self.store_suffixes {
            ...
            if !store_path.is_dir() {
                ...
                for child in store_path.read_dir().unwrap_or_else(|err| {
                    panic!("Error traversing Path: {path:?}, {err}", path = store_path)
                }) {
                    let child = child.unwrap_or_else(|err| {
                        panic!(
                            "Error while reading directory contents: {path:?}, {err}",
                            path = store_path
                        )
                    });
                    ...
                    .expect("could not write match to string");
                    consumer(&str_buf);
